...
export class CheckoutComponent {
  ...
  placeOrder():void{if(!this.items.length)return;this.submitting=true;this.success='';this.error='';this.http.post<any>('https://plantuno-backend.vercel.app/api/orders',{items:this.items.map(i=>({plantId:i.plantId,quantity:i.quantity})),}).subscribe({next:r=>{this.orderId=r.order._id;this.success='Order placed successfully!';this.cart.clear();this.refresh();this.submitting=false;},error:e=>{this.error=e?.error?.message||'Unable to place order. Please try again.';this.submitting=false;}})}
}
